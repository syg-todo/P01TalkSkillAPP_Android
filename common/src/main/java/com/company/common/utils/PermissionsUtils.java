    ...
    public void chekPermissions(Activity context, String[] permissions, @NonNull IPermissionsResult permissionsResult) {
        ...
        for (int i = 0; i < permissions.length; i++) {
            if (ContextCompat.checkSelfPermission(context, permissions[i]) != PackageManager.PERMISSION_GRANTED) {
                Log.e("PermissionsResult",permissions[i]);
...
//说明权限都已经通过，可以做你想做的事情去
          //  ToastUtils.showToast(context,"权限都有了");
    ...
    public void onRequestPermissionsResult(Activity context, int requestCode, @NonNull String[] permissions,
        ...
        if (mRequestCode == requestCode) {
            ...
            //    ToastUtils.showToast(context,"权限申请全部通过"+hasPermissionDismiss);

//如果有权限没有被允许
            if (hasPermissionDismiss) {
                //ToastUtils.showToast(context,"权限还有没申请通过的");
                ...
                //全部权限通过，可以进行下一步操作。。。
             //   ToastUtils.showToast(context,"权限申请全部通过");
